...
QUEUE=${QUEUE:-main}
qcn_cmd="qsub -cwd -j yes -V -S /bin/bash -q $QUEUE"
qsub_cmd="$qcn_cmd"
...
do
  ...
  do
	...
	do	
	  case $action in
	  "genAln")
	    $qsub_cmd -t 1-$NUM_SET ./wrapper-4-genAln $tip $diver $len $aln
	    ;;
	  "genFilter")
	    $qsub_cmd -t 1-$NUM_SET ./wrapper-4-genFilter $tip $diver $len $aln $method
	    ;;
	  "genTree")
	    $qsub_cmd -t 1-$NUM_SET ./wrapper-4-genTree $tip $diver $len $aln $method
	    ;;
	   "genML")
	    $qsub_cmd -t 1-$NUM_SET ./wrapper-4-genML $tip $diver $len $aln $method
	    ;;
	  esac
	done
    done
